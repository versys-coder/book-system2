...
import { fetchPoolWorkload, type PoolWorkloadResponse, type PoolWorkloadSlot } from "@app1/api-client/poolWorkload";

// Константы как в варианте 2
const BAR_WIDTH = 50;
const BAR_GAP = 5;
...
export default function Graph2() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        setError(null);
        const data: PoolWorkloadResponse = await fetchPoolWorkload({ start_hour: HOUR_START, end_hour: HOUR_END });
        if (!mounted) return;
        setSlots(data.slots || []);
      } catch (e: any) {
  ...
  const showPopup = (ev: React.MouseEvent, v: number, hour: number, isBreak: boolean, date: string) => {
  ...
  const hidePopup = () => {
    ...
    popupTimeout.current = window.setTimeout(() => setPopup(p => p ? { ...p, visible: false } : null), 80);
  };

  return (
    <div className="graph2-wrap" ref={containerRef}>
      <h1 className="graph2-title">Загруженность — график по свободным дорожкам</h1>

      <div className="graph2-grid-scroll">
        <div className="graph2-grid" style={{ paddingLeft: LEFT_PADDING, paddingTop: TOP_PADDING }}>
          {/* шапка часов */}
          <div className="graph2-hours">
            {hours.map(h => (
              <div key={h} className="graph2-hour">{String(h).padStart(2,"0")}:00</div>
            ))}
          </div>
              ...
              <div className="graph2-row" key={dateIso}>
                ...
                <div className="graph2-bars">
                  {hours.map(h => {
                    ...
                    const freeLanes = isBreak ? 0 : Math.max(0, Math.min(TOTAL_LANES, s?.freeLanes ?? 0));
                    const freePlaces = isBreak ? 0 : (s?.freePlaces ?? freeLanes * LANE_CAPACITY);

                    // Сегменты столбика сверху вниз (total lanes)
                    const segments = Array.from({ length: TOTAL_LANES }, (_, i) => {
                      // верхние сегменты считаем "свободными"
                      const idxFromTop = i; // 0..9
                      const isFree = idxFromTop < freeLanes;
                      const t = freeLanes <= 1 ? 1 : (freeLanes - idxFromTop) / freeLanes; // градиент внутри свободной зоны
                      const color = isFree ? lerpColor(COLOR_BOTTOM, COLOR_TOP, t) : INACTIVE_COLOR;
                      return (
                        <div
                          key={i}
                          className="graph2-seg"
                          style={{ background: isBreak ? INACTIVE_COLOR : color, height: SEGMENT_HEIGHT, marginBottom: SEGMENT_GAP }}
                        />
                      );
                    });
                    ...
                    return (
                      <div
                        key={`${dateIso}_${h}`}
                        className={`graph2-bar ${isBreak ? "is-break" : ""}`}
                        style={{ width: BAR_WIDTH, marginRight: BAR_GAP }}
                        onMouseEnter={(e) => showPopup(e, freeLanes, h, isBreak, dateIso)}
                      ...
                      >
                        <div className="graph2-bar-inner">
                          {segments}
                        </div>
                      </div>
                    );
        ...
        >
          {!popup.isBreak ? (
            <div className="poolworkload-popup">
              ...
              {/* без кнопки бронирования в graph2; можно добавить при необходимости */}
            </div>
          ) : (
